...
1. **Load your skills** by calling the `skill` tool for: `gather-context`, `interview`, `write-analysis`. Use `write-summary` if you need the expected structure of prior handoff documents. These contain your detailed instructions.
...
4. **Identify related prior work**:
   - Ask whether this task builds on an earlier SDD task, PR, or shipped feature
   - If yes, read `.opencode/docs/<prior-task>/summary.md` first when available
   - Use the prior task's `analysis.md` and `plan.md` only if the summary does not answer what changed
   - Capture reusable decisions, constraints, and touched files from the prior work

5. **Interview the user** (follow the `interview` skill):
   ...
   - Confirm whether prior work should be treated as precedent, dependency, or just reference material
   - Iterate until all blocking questions are resolved

6. **Write the analysis document** (follow the `write-analysis` skill):
...
7. **Present the document** to the user for final review before marking it as Final.
...
- If prior task summaries exist, you SHOULD read them before digging into older plans or code.
